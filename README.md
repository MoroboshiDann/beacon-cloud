...
​	短信平台负责和不同的运营商对接，每个运营商都对应一个通道Channel。系统中，给每个客户绑定了一定数量的短信通道，存储在数据库中。

​	当前过滤器首先查询缓存模块，获取客户绑定的通过通道信息。然后，根据权重选择当前可用的通道，给对应通道的消息对立中发送消息，等待网关模块消费。(一个网关模块实例，负责监听一个通道的消息队列)。
...
​	策略模块校验失败的短信、网关模块将短信发送到运营商后，会向MQ的SMS_WRITE_LOG队列中写入消息，等待搜索模块消费。

​	搜索模块监听MQ，有消息后就会消费。首先，获取当前短信对应的ES索引，然后向该索引中追加记录。使用的是ES提供的接口。

### 修改ES

​	运营商发送完成后，会给网关模块进行响应。网关模块修改短信的发送状态，并发消息至死信队列。

​	搜索模块监听SMS_GATEWAY_DEAD_QUEUE死信队列，获取短信对应的索引。先判断ES中是否存在该条记录，如果不存在就判断是否设置了重试，(因为此时记录不存在可能是由于先前写ES的消息还未被消费)，如果重试，就将该消息再次发送至死信队列，等待10s后再消费。

​	如果记录存在，就调用ES的API修改对应记录即可。
...
​	使用Netty来与运营商平台进行通信，Netty默认的线程组中有4个线程。当运营商发送回响应时，会默认使用线程组中的线程来执行任务，如写消息队列等。为了提高并发性能，我们要使用第三方线程池来管理线程。

​	我们使用Hippo4j来管理线程池。给两个任务分别设置一个动态线程池，一个负责初次写ES，一个负责接收到成功或失败响应后修改ES。当Netty收到响应后，根据响应的类型，来调用对应的线程池获取线程，执行我们设定好的任务(向MQ中写消息)。
...
​	为了保证写入ES的消息在修改ES的消息之前被消费，我们可以在RabbitMQ中设置死信队列。

​	首先，设置一个SMS_GATEWAY_NORMAL_EXCHANGE交换机，修改ES的请求都会先发送到这个交换机上，由其来路由到SMS_GATEWAY_NORMAL_QUEUE队列中。

​	然后，设置一个SMS_GATEWAY_NORMAL_QUEUE消息队列，这个队列没有消费者。并设置消息的生存时间，10s。生存时间一到，消息就会发送到SMS_GATEWAY_DEAD_EXCHANGE交换机。

​	SMS_GATEWAY_DEAD_EXCHANGE交换机将消息路由到SMS_GATEWAY_DEAD_QUEUE消息队列中，等待消费。

​	SMS_GATEWAY_DEAD_QUEUE死信队列，搜索模块负责监听并消费，修改对应的ES数据。

## RabbitMQ

​	默认情况下，消费者是单线程的，只有一个线程从消息队列中拉取消息进行消费，效率很低。可以通过配置，设置消费者线程数量以及线程单次拉取的消息数量。
