...
public class CmppSubmit extends CmppMessageHeader {
    ...
    public byte[] toByteArray() {
        ...
        buf.writeByte(1);
        buf.writeByte(0);
        buf.writeBytes(MsgUtil.getLenBytes(serviceId, 10));
        buf.writeByte((byte) 2);
        buf.writeBytes(MsgUtil.getLenBytes("", 21));
        buf.writeByte(0);
        buf.writeByte(0);
        buf.writeByte(8);

        buf.writeBytes(MsgUtil.getLenBytes(serviceId, 6));
        buf.writeBytes(MsgUtil.getLenBytes(feeType, 2));
        buf.writeBytes(MsgUtil.getLenBytes(feeCode, 6));
        buf.writeBytes(MsgUtil.getLenBytes(vaildTime, 17));
        buf.writeBytes(MsgUtil.getLenBytes(atTime, 17));
        buf.writeBytes(MsgUtil.getLenBytes(srcId, 21));
        buf.writeByte((byte) 1);
        buf.writeBytes(MsgUtil.getLenBytes(destTerminalId, 21));
