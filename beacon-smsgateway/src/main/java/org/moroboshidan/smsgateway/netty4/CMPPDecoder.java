...
import org.moroboshidan.smsgateway.netty4.entity.CmppActiveTestResp;
...
public class CMPPDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list){
        ...
        //4位 消息长度
        int totalLength = MsgUtil.bytesToInt(ArrayUtils.subarray(buf, 0, 4));
        //获取到该长度的字节数组
        byte[] bytes = ArrayUtils.subarray(buf, 0, totalLength);

        //获取到响应类型,也就是哪个接口的响应,4位
        int commandId = MsgUtil.bytesToInt(ArrayUtils.subarray(bytes, 4, 8));
